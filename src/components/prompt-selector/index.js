...
class PromptSelector extends Component {
  ...
  setLists() {
    ...
    typeList.forEach(
      (type) =>
        (typeObj[type] = _.filter(this.state.allPrompts, { type: type }))
    );
  ...
  onSelect(e) {
    ...
    const randomNr = Math.floor(
      Math.random() * this.state[selectedValue].length
    );
    const selectionResult = this.state[selectedValue][randomNr];
    const updateSelectedList = this.state[selectedValue].filter(
  ...
  render() {
    ...
    return (
      <div className="prompt-selector">
        ...
        <div>
          {selection.length > 0 && (
            <PromptGrid selection={selection} removeCard={this.removeCard} />
          )}
        </div>
